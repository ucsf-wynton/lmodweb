...
#      vers <- mixedsort(vers)
  ...
  if (length(idx) == 1L && is.finite(idx)) {
    ...
    ## The module does not have versions
    cstr(list(idx = idx, names = names, default = m[["version"]][idx], versions = m$versions[[1]], default = m$versions[[1]][idx, ], path=m$versions[[1]][idx,"path"], m = m), vec.len = 50L)
    stop("INTERNAL ERROR: Should not happen; the module does not have versions, or the identified default version does not actually exist")
  }
